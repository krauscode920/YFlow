...
class Dense(Layer):
    ...
    def backward(self, output_gradient):
        """Backward pass computing gradients with GPU support and 3D handling"""
        # Ensure gradient is on correct device
        output_gradient = self.device.to_device(output_gradient)
        
        # Handle 3D gradients (from transformers)
        gradient_original_shape = None
        if output_gradient.ndim == 3:
            # Store original gradient shape
            gradient_original_shape = output_gradient.shape  # (batch_size, seq_len, output_size)
            batch_size, seq_len, output_size = gradient_original_shape
            # Reshape to 2D: (batch_size * seq_len, output_size)
            output_gradient = output_gradient.reshape(-1, output_size)
        ...
        # Reshape input gradient back to 3D if input was 3D
        if self.original_shape is not None:
            batch_size, seq_len, features = self.original_shape
            input_gradient = input_gradient.reshape(batch_size, seq_len, features)
